...
final class _LaTeX extends _LaTeXToolBase {
  ...
  /** the description */
  private static final class __LaTeXDesc
  ...
  static final class _LaTexPathLoader {
    ...
    static {
      ...
      path = PathUtils.findFirstInPath(
          new AndPredicate<>(new FileNamePredicate(true, "latex" //$NON-NLS-1$
      ), CanExecutePredicate.INSTANCE), //
          IsFilePredicate.INSTANCE, visitFirst);
      if (path == null) {
        path = PathUtils.findFirstInPath(
            new AndPredicate<>(new FileNamePredicate(true, "cslatex" //$NON-NLS-1$
        ), CanExecutePredicate.INSTANCE), //
            IsFilePredicate.INSTANCE, visitFirst);
      }
