...
abstract class _LaTeXUsedAsToolBase extends _LaTeXToolChainComponent {
  ...
  _LaTeXUsedAsToolBase() {
    ...
    if (index > 0) {
      ...
      this.m_toolName = (this.m_tool1Name + " used as " //$NON-NLS-1$
          + this.m_tool2Name);
    } else {
      ...
      try {
        ...
        if (needsOutput) {
          if ((arg1 = args[index++]) == null) {
            throw new IllegalStateException(this.m_tool1Name + " binary '" //$NON-NLS-1$
                + exec
  ...
  final void _use(final _LaTeXMainJob job) throws IOException {
    ...
    if ((tex = this._getFile(job, ELaTeXFileType.TEX, true,
        null)) == null) {
      return;
    }
    ...
    ok = true;
    if (this._getFile(job, ELaTeXFileType.AUX, true,
