...
final class _Dvi2Ps extends _LaTeXToolChainComponent {
  ...
  _Dvi2Ps() {
    ...
    this.m_executable = PathUtils.findFirstInPath(
        new AndPredicate<>(new FileNamePredicate(true, "dvips", //$NON-NLS-1$
            "dvi2ps", //$NON-NLS-1$
            "dvitops"//$NON-NLS-1$
  ...
  final void _use(final _LaTeXMainJob job) throws IOException {
    ...
    if (this._getFile(job, ELaTeXFileType.PS, true,
  ...
  /** the description */
  private static final class __Dvi2PsDesc
