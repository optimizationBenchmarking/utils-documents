...
final class _Ps2Pdf extends _LaTeXToolChainComponent {
  ...
  _Ps2Pdf() {
    ...
    path = PathUtils.findFirstInPath(
        new AndPredicate<>(new FileNamePredicate(true, "pspdf", //$NON-NLS-1$
            "ps2pdf", //$NON-NLS-1$
            "pstopdf", //$NON-NLS-1$
            "pstopdf14", //$NON-NLS-1$
            "pstopdf13", //$NON-NLS-1$
    ...
    if (path == null) {
      path = PathUtils.findFirstInPath(
          new AndPredicate<>(new FileNamePredicate(true, "ps2pdf14"//$NON-NLS-1$
      ), CanExecutePredicate.INSTANCE), //
          IsFilePredicate.INSTANCE, visitFirst);
      if (path == null) {
        path = PathUtils.findFirstInPath(
            new AndPredicate<>(new FileNamePredicate(true, "ps2pdf13"//$NON-NLS-1$
        ), CanExecutePredicate.INSTANCE), //
            IsFilePredicate.INSTANCE, visitFirst);
        if (path == null) {
          path = PathUtils.findFirstInPath(
              new AndPredicate<>(new FileNamePredicate(true, "ps2pdf12"//$NON-NLS-1$
          ), CanExecutePredicate.INSTANCE), //
              IsFilePredicate.INSTANCE, visitFirst);
        }
      }
  ...
  final void _use(final _LaTeXMainJob job) throws IOException {
    ...
    if ((exec = this.m_executable) == null) {
      throw new UnsupportedOperationException(
          "No ps2pdf binary detected."); //$NON-NLS-1$
    }
  ...
  /** the description */
  private static final class __Ps2PdfDesc
