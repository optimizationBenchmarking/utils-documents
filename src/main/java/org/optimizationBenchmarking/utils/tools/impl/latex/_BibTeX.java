...
final class _BibTeX extends _LaTeXToolChainComponent {
  ...
  _BibTeX() {
    ...
    path = PathUtils.findFirstInPath(
        new AndPredicate<>(new FileNamePredicate(true, "bibtex", //$NON-NLS-1$
            "bibtex.original" //$NON-NLS-1$
    ), CanExecutePredicate.INSTANCE), //
        IsFilePredicate.INSTANCE, visitFirst);
    if (path == null) {
      path = PathUtils.findFirstInPath(
          new AndPredicate<>(new FileNamePredicate(true, "miktex-bibtex" //$NON-NLS-1$
      ), CanExecutePredicate.INSTANCE), //
          IsFilePredicate.INSTANCE, visitFirst);
    }
  ...
  final void _use(final _LaTeXMainJob job) throws IOException {
    ...
    if ((exec = this.m_executable) == null) {
      throw new UnsupportedOperationException(
          "No BibTeX binary detected."); //$NON-NLS-1$
    }

    if ((aux = this._getFile(job, ELaTeXFileType.AUX, true, //
  ...
  /** the description */
  private static final class __BibTeXDesc
