...
public abstract class DocumentDriver extends DocumentProducerTool
  ...
  protected void checkStyleForText(final IStyle style) {
    ...
    if (style instanceof IFontStyle) {
      return;
    }
    if (style instanceof IColorStyle) {
      if ((((IColorStyle) style).getColor().getRGB()
          & 0xffffff) != 0xffffff) {
        return;
      }
    }
   ...
   */
  protected Code createCode(final SectionBody owner, final ILabel useLabel,
   ...
   * @return the item
   */
  protected EnumerationItem createEnumerationItem(
   ...
   * @return the item
   */
  protected ItemizationItem createItemizationItem(
   ...
   */
  protected SubFigureCaption createSubFigureCaption(
  ...
  protected void reference(final ETextCase textCase,
    ...
    for (i = 0; i < len; i++) {
      ...
      if (i > 0) {
        ...
        runs[count++] = this.createReferenceRun(curType, sequenceMode,
            cpy);
        start = i;
      }
      curType = newType;
    ...
    this.referenceRuns(textCase, sequenceMode, //
