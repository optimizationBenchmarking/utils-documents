...
public final class LaTeXDocument extends Document {
  ...
  /** the default document class */
  private static final char[] REQUIRE_PACKAGE = { '\\', 'R', 'e', 'q', 'u',
      'i', 'r', 'e', 'P', 'a', 'c', 'k', 'a', 'g', 'e', };
  ...
  private static final char[] INCLUDE_GRAPHICS_1 = { '\\', 'i', 'n', 'c',
      'l', 'u', 'd', 'e', 'g', 'r', 'a', 'p', 'h', 'i', 'c', 's', '[', 'k',
      'e', 'e', 'p', 'a', 's', 'p', 'e', 'c', 't', 'r', 'a', 't', 'i', 'o',
      ',', 'w', 'i', 'd', 't', 'h', '=' };
  ...
  private static final char[] INCLUDE_GRAPHICS_PGF = { '\\', 'i', 'n', 'p',
      'u', 't', '{' };
  ...
  LaTeXDocument(final LaTeXDocumentBuilder builder) {
    ...
    this.m_setupPackagePath = PathUtils.createPathInside(
        this.getDocumentFolder(),
        ((PathUtils.getFileNameWithoutExtension(this.getDocumentPath())
            + "-setup.") + //$NON-NLS-1$
  ...
  protected final int getFiguresPerRow(final EFigureSize size) {
    ...
    perRowCalc = (int) (this.m_class.getUnit().convertToAsDouble(//
        (size.spansAllColumns() ? this.m_class.getWidth()
            : this.m_class.getColumnWidth()), //
        ELength.POINT) / //
        dim.getUnit().convertToAsDouble(dim.getWidth(), ELength.POINT));
        ...
        try {
          ...
          try {
            ...
            if (is != null) {
              ...
              if (ending != null) {
                findType: {
                  for (final EGraphicFormat format : EGraphicFormat.INSTANCES) {
                    if (ending
                        .equalsIgnoreCase(format.getDefaultSuffix())) {
                      type = format;
                      break findType;
                    }
                  }
              ...
              if ((logger != null) && (logger.isLoggable(Level.WARNING))) {
                logger.warning(((((("Resource '" //$NON-NLS-1$
                    + resource) + "' from class ") + //$NON-NLS-1$
                    TextUtils.className(clazz)) + //
  ...
  final String _pathRelativeToDocument(final Path path,
    ...
    if ((rel == null) || (rel.length() <= 0)) {
      throw new IllegalArgumentException(((("Relativizing '" + path) + //$NON-NLS-1$
          "' against document folder '") + folder)//$NON-NLS-1$
  ...
  final void _includeGraphics(final PhysicalDimension size,
      final ArrayListView<Map.Entry<Path, IGraphicFormat>> files,
      final ITextOutput out) {
    final String shrt;
    double width, height;
    ELength length;
    ...
    if (Compare.equals(EGraphicFormat.PGF, files.get(0).getValue())) {
      ...
      switch (length) {
        ...
        default: {
          width = length.convertToAsDouble(width, ELength.POINT);
          height = length.convertToAsDouble(height, ELength.POINT);
        ...
        try (final BufferedWriter bw = new BufferedWriter(osw)) {
          ...
          if (this.m_hasFigure) {
            ...
            if (this.m_hasFigureSeries) {
              ...
              LaTeXDriver._commentLine(
                  ...
                  out);
              this.__loadPackageAndRequire(out, "figureSeries.sty", //$NON-NLS-1$
                  null,
          ...
          if (this.m_hasUnderlined) {
            ...
            LaTeXDriver._commentLine("http://www.ctan.org/pkg/ulem", //$NON-NLS-1$
                out);
            this.__loadPackageAndRequire(out, "ulem.sty", //$NON-NLS-1$
  ...
  private final void __compile(
    ...
    try {
      ...
      for (final ImmutableAssociation<Path, IFileType> file : paths) {
        type = file.getValue();
        if ((type != null) && ((type instanceof EGraphicFormat)
            || (type instanceof ELaTeXFileType))) {
