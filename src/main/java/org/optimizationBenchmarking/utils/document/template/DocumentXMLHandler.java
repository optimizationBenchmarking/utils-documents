...
public final class DocumentXMLHandler extends DelegatingHandler {
  ...
  static {
    ...
    ReflectionUtils.addPackageOfClassToPrefixList(NumberAppender.class,
        paths);
    ReflectionUtils
        .addPackageOfClassToPrefixList(SimpleNumberAppender.class, paths);
    ReflectionUtils
        .addPackageOfClassToPrefixList(DecimalFormatSymbols.class, paths);
  ...
  private final void __startCall(final Attributes attributes) {
    ...
    if (callbackKey == null) {
      ...
      "Must specify attribute '" + //$NON-NLS-1$
          DocumentXML.ATTR_CALL_F)
          + "' to make a call to a callback via '") + //$NON-NLS-1$
  ...
  private final void __startCite(final Attributes attr) {
    ...
    sequenceMode = DocumentXML._parseSequenceMode(
    ...
    new BibliographyXMLHandler(this,
        this.__requireCurrentElement(IComplexText.class).cite(citationMode,
  ...
  private final void __startSection(final Attributes attributes) {
    ...
    sec = this.__requireCurrentElement(ISectionContainer.class)
        .section(label);
  ...
  private final void __startReference(final Attributes attributes) {
    ...
    this.m_formatTextCase = DocumentXML._parseTextCase(
  ...
  /**
   * Load formatting attributes: This will set the {@link #m_formatTextCase
   * text case} and the {@link #m_formatFormatter formatter}.
   *
   * @param atts
   *          the attributes
   */
  ...
  private final void __startMathComp(final Attributes attributes) {
    this.__pushElement(//
        this.__requireCurrentElement(IMath.class)
            .compare(DocumentXML._parseComparisonOperator(DelegatingHandler
